...
export async function generateCateringQuotePDF(
  ...
  // Colors - Califorian brand colors
  const primaryColor: [number, number, number] = [139, 90, 43]; // Rich brown/bronze
  const accentColor: [number, number, number] = [180, 130, 70]; // Warm gold
  ...
  doc.text("CALIFORIAN", margin, 28);
  
  // Tagline
  doc.setFont("helvetica", "italic");
  doc.setFontSize(11);
  doc.setTextColor(...accentColor);
  doc.text(t("Mediterranean Kitchen & Bar", "Akdeniz Mutfağı & Bar"), margin, 38);

  // Decorative elements
  doc.setDrawColor(...accentColor);
  doc.setLineWidth(0.5);
  doc.line(margin, 43, margin + 60, 43);
  ...
  doc.text(t("Quote Date", "Teklif Tarihi"), pageWidth - margin, 20, { align: "right" });
  doc.setFont("helvetica", "bold");
  doc.setTextColor(...textColor);
  doc.text(quoteDate, pageWidth - margin, 26, { align: "right" });
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(...primaryColor);
  doc.text(quoteNumber, pageWidth - margin, 41, { align: "right" });
