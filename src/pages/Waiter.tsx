...
const Waiter = () => {
  ...
  const updateItemQuantity = async (orderId: string, itemId: string, newQuantity: number) => {
    ...
    const newOrderTotal = (updatedItems || []).reduce((sum, i) => sum + i.total_price, 0);
    
    await supabase
      .from("orders")
      .update({ total: newOrderTotal, subtotal: newOrderTotal })
      .eq("id", orderId);

    toast({ title: t("Success", "Başarılı"), description: t("Item updated", "Ürün güncellendi") });
    setEditingItem(null);
    fetchOrders();
  };
  ...
  const deleteItem = async (orderId: string, itemId: string) => {
    ...
    if (!remainingItems || remainingItems.length === 0) {
      ...
      const newOrderTotal = remainingItems.reduce((sum, i) => sum + i.total_price, 0);
      await supabase
        .from("orders")
        .update({ total: newOrderTotal, subtotal: newOrderTotal })
        .eq("id", orderId);
    ...
    toast({ title: t("Success", "Başarılı"), description: t("Item deleted", "Ürün silindi") });
    ...
    <div className={`min-h-screen bg-background ${isFullscreen ? "fixed inset-0 z-50 overflow-auto" : ""}`}>
      ...
      <div className="bg-card border-b border-border p-3 sticky top-0 z-10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-sm">{staffSession.name}</span>
          </div>
          <div className="flex items-center gap-1">
            ...
            <Button onClick={handleLogout} variant="ghost" size="icon" className="h-9 w-9 text-destructive">
                      ...
                      <div key={item.id} className="flex items-center justify-between text-sm py-1 border-b border-border/30 last:border-0">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            ...
                            size="icon"
                            className="h-7 w-7"
                            onClick={() => { setEditingItem(item); setEditQuantity(item.quantity); }}
                          >
                          ...
                          <Button
                            ...
                            size="icon"
                            className="h-7 w-7 text-destructive"
                            onClick={() => setDeleteConfirm({ orderId: order.id, itemId: item.id })}
                          >
      ...
      <Dialog open={!!editingItem} onOpenChange={() => setEditingItem(null)}>
        <DialogContent className="max-w-xs">
          <DialogHeader>
            <DialogTitle>{t("Edit Quantity", "Miktar Düzenle")}</DialogTitle>
          </DialogHeader>
          {editingItem && (
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                {language === "tr" && editingItem.item_name_tr ? editingItem.item_name_tr : editingItem.item_name}
              </p>
              ...
              <Button
                className="w-full"
                onClick={() => {
                  const order = orders.find(o => o.items.some(i => i.id === editingItem.id));
                  if (order) {
                    updateItemQuantity(order.id, editingItem.id, editQuantity);
                  }
                }}
              >
                {t("Save", "Kaydet")}
      ...
      <AlertDialog open={!!deleteConfirm} onOpenChange={() => setDeleteConfirm(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("Delete Item?", "Ürün Silinsin mi?")}</AlertDialogTitle>
            <AlertDialogDescription>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => deleteConfirm && deleteItem(deleteConfirm.orderId, deleteConfirm.itemId)}
            >
              {t("Delete", "Sil")}
            </AlertDialogAction>
          </AlertDialogFooter>
